...
const MonitoringPanel = () => {
  ...
  const handleDelete = async (submissionId) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_APP_URL}/${submissionId}`,
        {
          method: "DELETE",
        }
      );
      ...
      setSubmissions((prevSubs) =>
        prevSubs.filter((sub) => sub._id !== submissionId)
      );
    } catch (err) {
  ...
  if (error)
    return (
      <p className="text-red-500 p-6 text-center text-lg font-semibold">
        {error}
      </p>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black to-gray-900 overflow-auto p-8 text-white select-none">
      ...
      <h2 className="text-4xl mb-6 font-extrabold text-center text-indigo-400 tracking-wide">
        📊 Monitoring Panel
      </h2>
      <p className="mb-10 text-center text-gray-300 text-lg max-w-3xl mx-auto">
        Live overview of student exam submissions — review answers and snapshots
        ...
        >
          ...
          >
            ...
            <div className="flex items-center space-x-2 text-indigo-400 font-medium">
              <span>
                {studentSubs.length} submission
                {studentSubs.length > 1 ? "s" : ""}
              </span>
              <FontAwesomeIcon
                icon={
                  expandedStudents[studentEmail] ? faChevronUp : faChevronDown
                }
              />
            </div>
          </header>
            ...
            <div className="space-y-10 transition-all duration-300 ease-in-out">
              {studentSubs.map((submission, idx) => {
                ...
                return (
                  <article
                    key={idx}
                    className="bg-gray-900 rounded-md p-6 border border-gray-700 shadow-md"
                  >
                        ...
                        <tbody>
                          {submission.answers.map((ans, i) => {
                            ...
                            return (
                              <tr
                                key={i}
                                className="even:bg-gray-700 odd:bg-gray-600 hover:bg-indigo-800 transition-colors"
                              >
                          ...
                          <div className="flex flex-wrap gap-3">
                            {submission.snapshots.map((snap, idx) => (
                              <div
                                ...
                                title={`Snapshot taken at: ${formatDateTime(
                                  snap.timestamp
                                )}`}
                              >
                                <img
